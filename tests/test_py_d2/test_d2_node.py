...
def test_d2_node_container():
    ...
    assert str(node) == "\n".join(["node_name: container_label {",
    "  node_1",
    "  node_2",
    "  node_1 -> node_2",
    "}"])
...
def test_d2_node_container_in_container_with_nodes():
    ...
    node = D2Node(name="node_name", label="container_label")
    node.add_node(D2Node(name="node_1", style=D2Style(fill="red"), nodes=[D2Node(name="node_2", style=D2Style(fill="blue")), D2Node(name="node_3")], links=[D2Link(from_node="node_2", to_node="node_3")]))
